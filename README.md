...
```sh
make docker-build docker-push IMG=<some-registry>/route-validator:tag
```
...
```sh
make deploy IMG=<some-registry>/route-validator:tag
```
...
```sh
make build-installer IMG=<some-registry>/route-validator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/route-validator/<tag or branch>/dist/install.yaml
```
